...
export async function harkList(country_id = 'all', provider = 'all') {
    const bibles = await fetch(`${BASE_CONTENT_URL}/index.json`).then(response => response.json());
    const output = bibles
    .map(bible => ({
        ...bible,
        tp: "hark",
        dl: (bible.dl) ? `${BASE_CONTENT_URL}/${bible.id}.zip` : ""
    }))
...
export async function harkSelect(id) {
    ...
    const filenameRequest = await fetch(`${BASE_CONTENT_URL}/${id}/index.txt`);
    ...
    let timingFiles = [];
    const response = await fetch(`${BASE_CONTENT_URL}/${id}/timingfiles/_index.txt`);
    if (response.ok) {
...
export async function setCurrentChapter(ctx, book, chapter) {
    ...
    if (ctx.currentType === 'hark') {
        ...
        ctx.currentChapter = {
            path: `https://audio.dbs.org/${ctx.currentBooks.bible_folder}/${book.book_number}_${book.name}_${safeChapter.toString().padStart(padLength, '0')}.mp3`,
        ...
        if (Array.isArray(ctx.currentBooks.timestamps) && ctx.currentBooks.timestamps.length > 0) {
            ...
            let timestampsText = await fetch(`https://audio.dbs.org/${ctx.currentBooks.bible_folder}/timingfiles/${ctx.currentBook.book_id}_${safeChapter.toString().padStart(3, '0')}.txt`).then(function (response) {
